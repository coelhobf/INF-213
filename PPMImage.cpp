...
istream &operator>>(istream &in, PPMImage &img)
{
	...
	for (int i = 0; i < img.rows; i++)
	{
		...
		{ //supomos que as imagens estao de cabeca para baixo (o pixel 0,0 e' o do canto inferior esquerdo, nao o do superior esquerdo)
	...
	/*for (int i = 0; i < img.rows; i++)
	{
		for (int j = 0; j < img.columns; j++)
		{ //supomos que as imagens estao de cabeca para baixo (o pixel 0,0 e' o do canto inferior esquerdo, nao o do superior esquerdo)
			in >> img.image[i][j].r >> img.image[i][j].g >> img.image[i][j].b;
		}
	}*/
	return in;
}
...
ostream &operator<<(ostream &out, const PPMImage &img)
{
	...
	/*for (int i = 0; i < img.getRows(); i++)
	{
		for (int j = 0; j < img.getColumns(); j++)
		{ //supomos que as imagens estao de cabeca para baixo (o pixel 0,0 e' o do canto inferior esquerdo, nao o do superior esquerdo)
			out << img.get(i, j).r << " " << img.get(i, j).g << " " << img.get(i, j).b << "  ";
		}
		out << "\n";
	}*/
	return out;
...
bool PPMImage::isUniform(int yMin, int xMin, int size, Color &representingColor) const
{
	...
	if (size == 1)
		return true; //regiao com 1 pixel certamente e' uniforme
