...
void testaInsert()
{
	...
	for (int i = 0; i < numInserts; i++)
	{
		...
		for (int i = 0; i < posInsert; i++, it = v.next(it))
			; //faca it apontar para a posicao "posInsert"
		v.insert(st, it);
...
void testaCompare()
{
	...
	for (int i = 0; i < numCompares; i++)
	{
		...
		for (int i = 0; i < a; i++, it1 = v.next(it1))
			; //faca it1 apontar para a posicao "a"

		MyList2<string>::iterator it2 = v.begin();
		for (int i = 0; i < b; i++, it2 = v.next(it2))
			; //faca it2 apontar para a posicao "b"
...
int main()
{
	...
	if (tipoTeste == 'E')
	{ //testes com funcao eraseMatchingElements...
