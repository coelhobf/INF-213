...
/*template<class T>
void MyList<T>::destroy(iterator it)
{ 
	if(it == NULL) return;
	if(it->next == NULL)
	{
	...
	this->destroy(it->next);
}*/

/*template<class T>
void MyList<T>::destroy()
{
	this->destroy(this->dataFirst);
...
void MyList<T>::push_front(const T&elem) {
	Node<T> *newN = new Node<T>(elem);

	newN->next = this->dataFirst;
	this->dataFirst = newN;
	this->dataSize++;
...
template<class T>
void MyList<T>::reverse(Node<T> *it)
{
	if(it == NULL) return;
	if(it->next == NULL)
	{
